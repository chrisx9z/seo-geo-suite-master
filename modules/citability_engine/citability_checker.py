...
def check_html(html: str, url: str = "") -> dict:
    """Analyze page HTML for GEO citability and passage readability."""
    return analyze(html, url)


def check_url(url: str) -> dict:
    """Fetch URL and analyze for GEO citability and structural readability."""
    try:
        html = fetch(url)
        return analyze(html, url)
        ...
        return {
            ...
            "applicable": False,
            "score": 0,
            "issues": [{"severity": "high", "finding": f"fetch failed: {exc}"}],
            "components": {},
        }
