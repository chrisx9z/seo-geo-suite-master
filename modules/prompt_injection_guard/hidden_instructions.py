...
def load_source(source: str, timeout: int = 30):
...
def check_url(url: str, timeout: int = 30) -> dict:
    """Check a live URL for hidden prompt injection instructions."""
    html, error = load_source(url, timeout=timeout)
    if error:
        return {
            "source": url,
            "error": error,
            ...
            "invisible_unicode": [],
            "score": 0,
            "issues": [f"Fetch error: {error}"],
            "limits": LIMITS,
        }
    return check_html(html, url)


find_hidden_instructions = check_html
